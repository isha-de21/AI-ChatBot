...
function FloatingChat({ open, onClose, theme }) {
...
function DailyCheckinModal({ open, onClose, onSave }) {
...
// Page Sections
function HomePage({ onNavigate, openCheckinModal }) {
  const [dailyQuote, setDailyQuote] = react.useState('');
...
function AwaazApp() {
  ...
  const [activeModulePage, setActiveModulePage] = react.useState(null);
  ...
  const handleNavigate = (pageKey) => {
    const modulePages = ["journalingPage", "meditationPage", "copingPage", "breathingPage", "learnPage"];
    if (modulePages.includes(pageKey)) {
      setActive("modules");
  ...
  const renderPage = () => {
    switch (active) {
      ...
      case "modules":
        // Fix is here: use the correct key string "journalingPage"
